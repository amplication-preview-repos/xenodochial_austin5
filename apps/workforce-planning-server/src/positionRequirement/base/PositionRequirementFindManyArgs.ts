/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { PositionRequirementWhereInput } from "./PositionRequirementWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { PositionRequirementOrderByInput } from "./PositionRequirementOrderByInput";

@ArgsType()
class PositionRequirementFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => PositionRequirementWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => PositionRequirementWhereInput, { nullable: true })
  @Type(() => PositionRequirementWhereInput)
  where?: PositionRequirementWhereInput;

  @ApiProperty({
    required: false,
    type: [PositionRequirementOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [PositionRequirementOrderByInput], { nullable: true })
  @Type(() => PositionRequirementOrderByInput)
  orderBy?: Array<PositionRequirementOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { PositionRequirementFindManyArgs as PositionRequirementFindManyArgs };
